resource "google_container_cluster" "this" {
  ...
  addons_config {
    horizontal_pod_autoscaling {
      disabled = false == var.horizontal_pod_autoscaling
    }
    http_load_balancing {
      disabled = false == var.http_load_balancing
    }
    kubernetes_dashboard {
      disabled = false == var.kubernetes_dashboard
    }
    istio_config {
  ...
  # The empty values here disable a password rather than set an empty one.
